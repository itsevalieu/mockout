...
const topics = ["javascript", "react", "frontend", "css", "node"];
...
const Form = () => {
  ...
  const detectTopics = (hashedTextArray: any, topics: string[]) => {
    ...
    const p = new Promise<string[]>(async (resolve, reject) => {
      let cleanedTopics: string[] = await topics.filter((topic) =>
        hashedTextArray.has(topic)
      );
      ...
      <div className="container">
        <form onSubmit={handleFormSubmit}>
          ...
          <textarea
            ...
            ref={textAreaRef}
            placeholder="Insert Job Description text"
            autoFocus
          />
          <button type="submit">Generate</button>
        </form>
        {isLoading ? null : (
          <ItemList filteredTopics={filteredTopics} items={items} />
        )}
      </div>
    </main>
  );
